...
public class PlayerState : MonoBehaviour
{
	...
	[SerializeField] private float verticalPosition = 1.7f;
	[SerializeField] private float currentSpeed = 1;
	Vector3[] climbingPositions;
	...
	public void Awake()
	{
		...
		climbingPositions = new Vector3[] {
			new Vector3(-1, verticalPosition),
			new Vector3(-1, verticalPosition),
			new Vector3(0, verticalPosition),
			new Vector3(1, verticalPosition),
			new Vector3(1, verticalPosition),
		};
	...
	private void Start()
	{
		transform.position = climbingPositions[2];
	...
	IEnumerator ShortJump(Vector3 destination)
	{
		isMidAir = true;
		...
		while (time < 1)
		{
			...
			transform.position = Vector3.Slerp(originPosition, destination, xAnimationCurve.Evaluate(time));
	...
	private void ShortJump(TouchState wholeTouch)
	{
		...
		int positionIndex = ((int)horizontalPosition + direction) + 1;

		StartCoroutine(ShortJump(climbingPositions[positionIndex]));

		//Action jumpAction = jumpActions[functionIndex];

		//jumpAction.Invoke();

		horizontalPosition = (Tower)climbingPositions[positionIndex].x;
